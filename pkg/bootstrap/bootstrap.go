/*
Copyright 2023 The Tekton Authors

Licensed under the Apache License, Version 2.0 (the "License");
you may not use this file except in compliance with the License.
You may obtain a copy of the License at

http://www.apache.org/licenses/LICENSE-2.0

Unless required by applicable law or agreed to in writing, software
distributed under the License is distributed on an "AS IS" BASIS,
WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
See the License for the specific language governing permissions and
limitations under the License.
*/

package bootstrap

import (
	"context"
	"fmt"

	triggersclientset "github.com/tektoncd/triggers/pkg/client/clientset/versioned"
	"k8s.io/client-go/kubernetes"
)

// Bootstrapper handles the bootstrap process
type Bootstrapper struct {
	config           *Config
	kubeClient       kubernetes.Interface
	triggersClient   triggersclientset.Interface
	installer        *Installer
	rbacManager      *RBACManager
	templatesManager *TemplatesManager
	githubManager    *GitHubManager
}

// New creates a new bootstrapper
func New(config *Config) (*Bootstrapper, error) {
	var kubeClient kubernetes.Interface
	var triggersClient triggersclientset.Interface
	var err error

	// For dry-run mode, we don't need real clients
	if !config.DryRun {
		// Create Kubernetes clients
		kubeClient, err = kubernetes.NewForConfig(config.KubeConfig)
		if err != nil {
			return nil, fmt.Errorf("failed to create kubernetes client: %w", err)
		}

		triggersClient, err = triggersclientset.NewForConfig(config.KubeConfig)
		if err != nil {
			return nil, fmt.Errorf("failed to create triggers client: %w", err)
		}
	}

	// Create component managers
	installer := NewInstaller(kubeClient, config)
	rbacManager := NewRBACManager(kubeClient, config)
	templatesManager := NewTemplatesManager(triggersClient, config)
	githubManager := NewGitHubManager(config)

	return &Bootstrapper{
		config:           config,
		kubeClient:       kubeClient,
		triggersClient:   triggersClient,
		installer:        installer,
		rbacManager:      rbacManager,
		templatesManager: templatesManager,
		githubManager:    githubManager,
	}, nil
}

// Run executes the bootstrap process
func (b *Bootstrapper) Run(ctx context.Context) error {
	if b.config.Verbose || b.config.DryRun {
		fmt.Println("Bootstrap Configuration:")
		fmt.Printf("  Namespace: %s\n", b.config.Namespace)
		fmt.Printf("  Provider: %s\n", b.config.Provider)
		fmt.Printf("  Create Examples: %v\n", b.config.CreateExamples)
		fmt.Printf("  Setup Ingress: %v\n", b.config.SetupIngress)
		fmt.Printf("  Dry Run: %v\n", b.config.DryRun)
		fmt.Println()
	}

	// Install dependencies
	if b.config.InstallDeps {
		// Check and install Tekton Pipelines
		fmt.Println("====Checking Tekton Pipelines...")
		if err := b.installer.InstallTektonPipelines(ctx); err != nil {
			return fmt.Errorf("failed to ensure pipelines: %w", err)
		}

		// Install Tekton Triggers
		if !b.config.SkipInstall {
			fmt.Println("=====Checking Tekton Triggers...")
			if err := b.installer.InstallTriggers(ctx); err != nil {
				return fmt.Errorf("failed to install triggers: %w", err)
			}
		}

		fmt.Println("=-=-=-= Tekton dependencies installed! =-=-=-=\n")
	}

	fmt.Println("====Setting up Trigger resources...")

	// Create/verify namespace 
	fmt.Printf(" ====Setting up namespace '%s'...\n", b.config.Namespace)
	if err := b.rbacManager.CreateNamespace(ctx); err != nil {
		return fmt.Errorf("failed to create namespace: %w", err)
	}

	// Set up RBAC
	if b.config.InstallDeps {
		fmt.Println(" ====Setting up RBAC...")
		if err := b.rbacManager.SetupRBAC(ctx); err != nil {
			return fmt.Errorf("failed to setup RBAC: %w", err)
		}

		// Create Trigger resources (EventListener, TriggerTemplate, TriggerBinding)
		fmt.Println(" ====Creating Trigger resources...")
		if err := b.templatesManager.CreateTriggerResources(ctx); err != nil {
			return fmt.Errorf("failed to create trigger resources: %w", err)
		}
	}

	// Create examples (Pipeline)
	if b.config.CreateExamples {
		fmt.Println(" ====Creating example Pipeline...")
		if err := b.templatesManager.CreateExamples(ctx); err != nil {
			return fmt.Errorf("failed to create examples: %w", err)
		}
	}

	// Setup ingress if requested
	if b.config.SetupIngress {
		fmt.Println("Setting up ingress...")
		if err := b.rbacManager.SetupIngress(ctx); err != nil {
			return fmt.Errorf("failed to setup ingress: %w", err)
		}
	}

	// Check existing webhooks
	if b.config.SetupIngress && b.config.GitHubToken != "" {
		fmt.Println(" ====Checking existing webhooks...")
		exists, err := b.githubManager.CheckExistingWebhooks(ctx)
		if err != nil {
			return fmt.Errorf("failed to check webhooks: %w", err)
		}
		if exists && !b.config.ForceSetup {
			fmt.Println("## Webhook already exists (use --force-setup to recreate)")
		} else {
			// Create GitHub webhook
			fmt.Println(" ====Creating GitHub webhook...")
			if err := b.githubManager.SetupWebhook(ctx); err != nil {
				return fmt.Errorf("failed to setup webhook: %w", err)
			}
		}
	}

	// Create webhook secret in Kubernetes
	if b.config.SetupIngress && b.githubManager.GetWebhookSecret() != "" {
		fmt.Println(" ====Creating webhook secret...")
		if err := b.rbacManager.CreateWebhookSecret(ctx, b.githubManager.GetWebhookSecret()); err != nil {
			return fmt.Errorf("failed to create webhook secret: %w", err)
		}
	}
	return nil
}
